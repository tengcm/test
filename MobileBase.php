...
//        Session::start();
        ...
        // 判断当前用户是否手机                
        if(isMobile())
            cookie('is_mobile','1',3600*24*365); 
        else 
            cookie('is_mobile','0',3600*24*365);
        
        $is_alipay = 0;
        $first_leader = I('first_leader');
        session('first_leader',$first_leader);
        $tmobile = I('tmobile');
        session('tmobile',$tmobile);
        $first_leader = session('first_leader');
            // print_r($first_leader);
            // print_r($tmobile);
            // die;
        ...
        if(strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger')){
            ...
            if (empty($_SESSION['openid'])) {
                ...
                if(is_array($this->weixin_config) && $this->weixin_config['wait_access'] == 1){
                    ...
                    if($data['status'] == 1){
                        ...
                        setcookie('user_id',$data['result']['user_id'],3600*24*365,'/');
                        setcookie('is_distribut',$data['result']['is_distribut'],3600*24*365,'/');
                        setcookie('uname',$data['result']['nickname'],3600*24*365,'/');
            ...
            $myUserId = cookie('user_id');
            if($myUserId){
                
                $myuser = M('users')->where(['user_id'=>$myUserId])->find();
                session('user',$myuser);
            }
            ...
            if(isAliClient()){
                ...
                if($myUserId){
                    ...
                    $now_url = $this->get_url();
                    
                    $is_alipay = 1;
                    // echo $is_alipay;
                    // return $this->redirect(U('Mobile/Aliclub/index',['first_leader'=>$first_leader]));
